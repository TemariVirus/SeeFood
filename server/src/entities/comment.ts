...
export class Review implements Entity {
  ...
  static readonly requiredColumns: (keyof Review)[] = [
    "rating",
    "restaurant_id",
    "date",
    "user_id",
...
export class Reply implements Entity {
  ...
  static readonly requiredColumns: (keyof Reply)[] = [
    "content",
    "date",
    "user_id",
    "review_id",
...
export class Comment {
  ...
  static {
    Query.prototype.toCommentArray = function (this: Query<any>) {
      return Query.select("r.*", "username")
        ...
        .join(
          JoinType.INNER_JOIN,
          Query.select("id", "`name` AS username").from(User).as("u"),
          "r.user_id",
          SqlOperators.EQUAL,
          "u.id"
        )
  ...
  static fromNewRequest(request: z.infer<typeof newCommentRequest>) {
    ...
    try {
      ...
      const { content, rating, parent_id, user_id, is_reply } =
        newCommentRequest.parse(request);
      // Create comment
      const comment = is_reply ? new Reply() : new Review();
      comment.content = content;
      comment.user_id = user_id;
      comment.date = new Date(Date.now());
      if (comment instanceof Reply) comment.review_id = parent_id;
      else {
  ...
  static fromUpdateRequest(request: z.infer<typeof updateCommentRequest>) {
    ...
    try {
      ...
      if (comment instanceof Review && rating) comment.rating = rating;
